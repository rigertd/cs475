...
main( int argc, char *argv[ ] )
{
        ...
        // host=CPU, not GPU
        // Can be global or heap, but do not do local.
        ...
        // (properties, # of devices, device pointer, callback, user data pointer, returned status)

        cl_context context = clCreateContext( NULL, 1, &device, NULL, NULL, &status );
        if( status != CL_SUCCESS )
                fprintf( stderr, "clCreateContext failed\n" );

        // 4. create an opencl command queue: (where you send things, each device can have its own queue)
        // (context, device, properties, returned status)

        cl_command_queue cmdQueue = clCreateCommandQueue( context, device, 0, &status );
        if( status != CL_SUCCESS )
                fprintf( stderr, "clCreateCommandQueue failed\n" );

        // 5. allocate the device memory buffers: (device=GPU)
        // (context, buffer restrictions, dataSize (bytes), buffer data already allocated, returned status)
        // READ/WRITE is with respect to GPU device. READ means GPU will only read the memory
        // WRITE means GPU will only write to the memory
        // CL_MEM_READ_WRITE means GPU will do both
        // d<varname> = deviceVarname, h<varname> = hostVarname (professor's convention)
        ...
        // (command queue, device buffer, block until done?, offset, #bytes to write, host buffer, # events, event wait list, event object)
        // CPU can stuff as much as it wants into the queue, GPU will get to it when it can (async)
        ...
        // Waits for the command queue to finish executing all commands
        Wait( cmdQueue );
